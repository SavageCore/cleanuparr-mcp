...
of one tool per endpoint. Each portmanteau tool (e.g. cleanuparr_events,
cleanuparr_cleaner_config) takes an `operation` enum plus an `arguments`
dict; see AGENTS.md for the rationale (an 80+-tool server blows the MCP
context budget on session start). `_GROUPS` near the bottom buckets every
endpoint function by resource and `_register_group` wraps each bucket in one
...
async def cleanuparr_update_job_schedule(job_type: str, schedule: dict) -> JSONObj:
    ...
    return await _req(
        "PUT", f"/Jobs/{_path(job_type)}/schedule", {"schedule": schedule}
    )
...
async def cleanuparr_get_stats(
    hours: int = 168, include_dry_run: bool = False
) -> JSONObj:
    """Get v2 aggregate statistics. The deprecated v1 stats endpoint is not exposed."""
    return await _req(
        "GET", "/v2/stats", params={"hours": hours, "includeDryRun": include_dry_run}
    )
...
async def cleanuparr_list_search_events(
    ...
    search_status: list[str] | None = None,
...
async def cleanuparr_list_cf_scores(
    ...
    return await _req(
        ...
        params=_params(
            ...
            itemType=item_type,
            cutoffFilter=cutoff_filter,
            monitoredFilter=monitored_filter,
        ),
    )
...
async def cleanuparr_list_cf_upgrades(
    ...
    days: int = 30,
    search: str | None = None,
    sort_by: str = "UpgradedAt",
    sort_direction: str | None = None,
) -> JSONObj:
    """List recent custom-format score upgrades."""
    return await _req(
        "GET",
        "/seeker/cf-scores/upgrades",
        params=_params(
            page=page,
            pageSize=page_size,
            instanceId=instance_id,
...
async def cleanuparr_get_cf_item_history(
    instance_id: str, item_id: int, episode_id: int = 0
) -> JSONObj:
    """Get score history for an item; episode_id is used for Sonarr episode history."""
    return await _req(
        "GET",
        f"/seeker/cf-scores/{_path(instance_id)}/{item_id}/history",
...
async def cleanuparr_update_arr_instance(
    arr_type: str, instance_id: str, instance: dict
) -> JSONObj:
    """Replace an *arr instance. Use Cleanuparr's secret placeholder for apiKey."""
    return await _req(
        "PUT",
        f"/configuration/{_path(arr_type)}/instances/{_path(instance_id)}",
        instance,
    )
...
async def cleanuparr_delete_arr_instance(arr_type: str, instance_id: str) -> JSONObj:
    ...
    return await _req(
        "DELETE", f"/configuration/{_path(arr_type)}/instances/{_path(instance_id)}"
    )
...
async def cleanuparr_test_arr_instance(arr_type: str, instance: dict) -> JSONObj:
    ...
    return await _req(
        "POST", f"/configuration/{_path(arr_type)}/instances/test", instance
    )
...
async def cleanuparr_update_download_client(client_id: str, client: dict) -> JSONObj:
    ...
    return await _req(
        "PUT", f"/configuration/download_client/{_path(client_id)}", client
    )
...
async def _queue_rule(
    method: str, kind: str, rule_id: str | None = None, rule: dict | None = None
) -> JSONVal:
...
async def _client_config(
    path: str, client_id: str, method: str = "GET", config: dict | None = None
) -> JSONVal:
...
async def cleanuparr_update_dead_torrent_config(
    download_client_id: str, config: dict
) -> JSONObj:
    """Replace dead-torrent settings: enabled, targetCategory, useTag, maxStrikes, categories."""
    return await _client_config(
        "/dead-torrent-config", download_client_id, "PUT", config
    )
...
async def cleanuparr_update_unlinked_config(
    download_client_id: str, config: dict
) -> JSONObj:
...
async def cleanuparr_update_orphaned_files_config(
    download_client_id: str, config: dict
) -> JSONObj:
    ...
    return await _client_config(
        "/orphaned-files-config", download_client_id, "PUT", config
    )
...
async def cleanuparr_create_seeding_rule(
    download_client_id: str, rule: dict
) -> JSONObj:
...
async def cleanuparr_reorder_seeding_rules(
    download_client_id: str, ordered_ids: list[str]
) -> JSONObj:
    """Reorder rules by sending an orderedIds GUID array."""
    return await _req(
        "PUT",
        f"/seeding-rules/{_path(download_client_id)}/reorder",
...
async def cleanuparr_create_notification_provider(
    provider_type: str, provider: dict
) -> JSONObj:
    ...
    return await _req(
        "POST",
        f"/configuration/notification_providers/{_path(provider_type)}",
        provider,
    )


async def cleanuparr_update_notification_provider(
    provider_type: str, provider_id: str, provider: dict
) -> JSONObj:
    """Replace a notification provider. Use Cleanuparr placeholders for secrets."""
    return await _req(
        "PUT",
        f"/configuration/notification_providers/{_path(provider_type)}/{_path(provider_id)}",
        provider,
    )


async def cleanuparr_test_notification_provider(
    provider_type: str, provider: dict
) -> JSONObj:
    """Test a notification provider using its provider-specific DTO."""
    return await _req(
        "POST",
        f"/configuration/notification_providers/{_path(provider_type)}/test",
        provider,
    )
...
async def cleanuparr_delete_notification_provider(provider_id: str) -> JSONObj:
    ...
    return await _req(
        "DELETE", f"/configuration/notification_providers/{_path(provider_id)}"
    )
...
def _register_group(
    ...
    mcp.add_tool(
        Tool.from_function(
            ...
            name=group,
            description=(
                f"{group.replace('_', ' ')} operations on Cleanuparr. Pass `operation` and an "
...
def main() -> None:
    ...
    if not url:
        print(
            "CLEANUPARR_URL environment variable is required (e.g. https://cleanuparr.example.com)",
            file=sys.stderr,
        )
        raise SystemExit(1)
